...
class UserController {
    ...
    //         }
    //         if (password !== password_confirmation) {
    //             return res.status(403).send({ state: "failed", message: "password and confirm password not matched" });
    //         }
    //         const salt = await bcrypt.genSalt(10);
    //         const hashPassword = await bcrypt.hash(password, salt);
    ...
    //         const link = `http://127.0.0.1:5000/api/user/reset-password/${user._id}/${token}`
    //         // console.log(link)
